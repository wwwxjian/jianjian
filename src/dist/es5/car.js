    ...
    */
    function cartext() {
        ...
        $('#car').on('click', '.add', function () {
            ...
            var type = 'add_btn';
            var id = $(this).parent().parent().parent().attr('date-id');
            var data = 'type=' + type + '&number=' + val + '&goodsid=' + id;
        ...
        $('#car').on('click', '.dele', function () {
            ...
            if (res) {
                ...
                erzi.parent().remove();
                // 访问购物车数据库
                var deleId = $(this).parent().parent().attr('date-id');
                // console.log(deleId);
                var type = 'deleOne';
                var _url = '../api/car.php';
                var _data = 'type=' + type + '&goodsid=' + deleId;
        ...
        $('#car').on('click', '.deleAll', function () {
            ...
            if (res) {
                for (var i = arr.length - 1; i >= 0; i--) {
                    var id = $('#car_goods tr').eq(i).attr('date-id');
                    // console.log(id);
                    var type = 'deleOne';
                    var _url2 = '../api/car.php';
                    var _data2 = 'type=' + type + '&goodsid=' + id;
                    ajax('get', _url2, _data2, function (str) {
                        var arr = JSON.parse(str);
                        // console.log(arr);
                    });
