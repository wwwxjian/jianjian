...
$(function () {
    ...
    var passw = getid('passw');
    var repassw = getid('repassw');
    var check1 = getid('check1');
    var check2 = getid('check2');
    var drag = getid('drag');
    var regi = getid('regi');
        ...
        if (user) {
            if (checkReg.tel(user) || checkReg.email(user)) {
                ...
                drag.onblur = function () {
                    var val = drag.value.trim();
                    var vals = inputs[2].value.trim();
                    if (val) {
                        if (val == vals) {
                            spans[0].innerText = '';
                            css(drag, 'border', '');
